...
void leerMallaYCondiciones(mesh &m,char *filename){
    ...
    ifstream file;
    float zero,f_x,f_y,f_z;
    int nnodes, neltos, ndirich_u, ndirich_v;
    condition *dirichlet_u, *dirichlet_v;

    addExtension(inputfilename,filename,".dat");
    file.open(inputfilename);

    file >> zero >> f_x >> f_y >> f_z;
    
    file >> nnodes >> neltos >> ndirich_u >> ndirich_v;

    m.setParameters(zero,f_x,f_y,f_z);
    m.setSizes(nnodes,neltos,ndirich_u+ndirich_v);
    m.createData();
    ...
    fusionDirichlet(ndirich_u,dirichlet_u,ndirich_v,dirichlet_v,m.getDirichlet());
    
    correctConditions(ndirich_u+ndirich_v,m.getDirichlet(),m.getDirichletIndices());
}
